...
public class Player : MonoBehaviour
{
    ...
    void SetScoreText()
    {
        ...
        if (score >= 25) SceneManager.LoadScene(3); // Ef leikmaður fær 25 stig eða meira þá lýkur leiknum og leikmaður hefur unnið.
    }
    void SetHealthText()
    {
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject.CompareTag("BigPickUp"))
        {
            collision.gameObject.SetActive(false);
            score = score + 25; // Gefur leikmanni 25 stig fyrir stóra peninginn
            health = health + 30; // Gefur leikmanni 30 líf fyrir að ná stóra peningnum
            SetScoreText(); // Sendir á fall að uppfæra UI texta
            SetHealthText(); // Sendir á fall að uppfæra UI texta
        }
        ...
        {
            TakeDamage(1000); // Spilari missir allt líf og leikurinn er tapaður
        }
    }
